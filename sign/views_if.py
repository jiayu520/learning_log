...
def get_guest_list(request):
    ...
    if eid != '' and phone =='':
        datas = []
        results = Guest.objects.filter(evemt_id=eid)
        if results:
            for r in results:
                guest = {}
            ...
            return JsonResponse({'status':10022,'messag':'query result is empty'})
...
def user_sign(request):
    ...
    if not result:
        return JsonResponse({'status':10026,'message':'user did not participat in the conference'})
